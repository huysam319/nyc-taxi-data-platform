...
def transform_bronze_to_silver(
    ...
    active_batch_id = (
        batch_id
        or f"silver-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}-{uuid4().hex[:8]}"
    )
    ...
    for year, month in bronze_partitions:
        if _is_already_transformed(silver_dir, year, month):
            logger.info(
                "status=SKIP_PARTITION batch_id=%s partition='year=%d/month=%d' reason='Already exists in Silver'",
                active_batch_id,
                year,
                month,
            )
        ...
        logger.info(
            "status=FINISHED_PARTITION batch_id=%s partition='year=%d/month=%d' duration_sec=%.2f records_written=%d",
            active_batch_id,
            year,
            month,
            partition_duration,
    ...
    logger.info(
        "status=SUCCESS_SILVER batch_id=%s total_duration_sec=%.2f total_records_written=%d partitions_skipped=%d",
        active_batch_id,
        job_duration,
